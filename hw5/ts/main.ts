...
const areaCircle = (r:number) => (Math.PI * Math.pow(r, 2));

console.log(areaCircle(4));


// - створити функцію яка обчислює та повертає площу циліндру висотою h, та радіутом r
const areaCylinder = (h:number, rad:number) => 2 * Math.PI * rad * (h + rad);

console.log(areaCylinder(3, 5));

// - створити функцію яка приймає масив та виводить кожен його елемент
const returnElementOfArr = (array:(number | boolean | string)[]) => {
...
const createP = (text:string) => document.write(`<p>${text}</p>`);
...
const createListSecond = (text:string, number:number) => {
    document.write(`<ul>`)
...
const createListOfArr = (arr:(number | boolean | string)[]) => {
    document.write(`<ul>`);
...
const returnProperti = (arr:Users[]) => {
...
const returnMinNumber = (arr:number[]) => {
    ...
    document.write(String(minNumber));
}
...
const swap2 = (arr:number[], index1:number, index2:number) => {
    const newArr2 = [];
    for (let i:number = 0; i < arr.length; i++) {
        if (i === index1) {
...
const exchange = (sumUAH:number, currencyValues:Currency[], exchangeCurrency:string) => {
    let result;
