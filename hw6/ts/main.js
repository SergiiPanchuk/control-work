...
var arrNumbers = [10, 8, -7, 55, 987, -1011, 0, 1050, 0];
var arrStrings = arrNumbers.map(function (x) { return x.toString(); });
...
var filterArr = sortArr.filter(function (a) { return a.monthDuration > 5; });
var addValueArr = filterArr.map(function (x, index) {
    x['id'] = index + 1;
    return x;
});
...
function createCards(cardSuit, value) {
    for (var _i = 0, cardSuit_1 = cardSuit; _i < cardSuit_1.length; _i++) {
        var cardSuitElement = cardSuit_1[_i];
        for (var _a = 0, value_1 = value; _a < value_1.length; _a++) {
            var valueElement = value_1[_a];
            cards.push("".concat(valueElement, " ").concat(cardSuitElement));
        }
    }
}
...
console.log(cards);
// - знайти піковий туз
console.log(cards.find(function (x) { return x === 'ace spade'; }));
// - всі шістки--target es6 --lib es5
var allSix = cards.filter(function (x) {
    return x.includes('6');
});
console.log(allSix);
// - всі червоні карти
var allHeart = cards.filter(function (x) { return x.includes('heart'); });
var allDiamond = cards.filter(function (x) { return x.includes('diamond'); });
console.log(allHeart.concat(allDiamond));
// - всі буби
console.log(allDiamond);
// - всі трефи від 9 та більше
var allClubs = cards.filter(function (x) {
...
//     clubs:[]
// }
var result = cards.reduce(function (accumulator, x) {
    if (x.includes('spade')) {
...
console.log(findAllSassModules(coursesArray));
console.log(findAllDockerModules(coursesArray));
