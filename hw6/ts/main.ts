...
const arrNumbers: number[] = [10, 8, -7, 55, 987, -1011, 0, 1050, 0];
const arrStrings: string[] = arrNumbers.map(x => x.toString())
...
function sortNums(nums: number[], ascending: string) {
...
const sortArr: Courses[] = coursesAndDurationArray.sort(function (a, b) {
    return a.monthDuration - b.monthDuration;
})
const filterArr: Courses[] = sortArr.filter(a => a.monthDuration > 5);
const addValueArr: Courses[] = filterArr.map(function (x, index: number) {
    x['id'] = index + 1;
    return x;
})
...
function createCards(cardSuit: string[], value: (string | number)[]) {
